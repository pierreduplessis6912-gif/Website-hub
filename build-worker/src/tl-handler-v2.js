...
async function handleDownloadProductRun(url, env) {
  ...
  try {
    ...
    const tenderRef = (report.tender_reference || '').replace(/\//g, '-').replace(/[^a-zA-Z0-9\-]/g, '').slice(0, 30).trim();
...
- CRITICAL FRAUD RISK WARNING: If company profile data is contradictory — specifically where compliance documents are claimed (e.g. "COIDA good standing", "R2m insurance") but marked "Not on file" — this is not a data entry issue. This is a PRECCA fraud risk. Under the Prevention and Combating of Corrupt Activities Act (Act 12 of 2004) Section 34, government evaluation committees are legally obligated to report such contradictions to the Hawks. Directors face criminal prosecution. The company faces permanent blacklisting on the Register for Tender Defaulters. Flag this explicitly as PRECCA_FRAUD_RISK in the compliance checklist. Use the word "fraud risk" — not "data integrity issue". Recommend withdrawal if documents cannot be genuinely obtained before closing date. State clearly: submitting with unresolved contradictions is a criminal risk, not an administrative one.
