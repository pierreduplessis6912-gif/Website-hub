...
async function handleDomainCheck(url, env) {
  ...
  try {
    ...
    let suggestions = [];
    if (!available) {
      const alts = [sld + '-sa', sld.replace(/-/g, ''), sld + '-' + new Date().getFullYear().toString().slice(-2)]
      ...
      suggestions = await Promise.all(alts.map(async altSld => {
        try {
          ...
          return { domain: `${altSld}.co.za`, available: altData?.result === 'available' || altData?.available === true };
...
async function callDomainProxy(action, sld, tld = 'co.za', extra = {}, env) {
  ...
  // Token: base64(HMAC-SHA256(apiKey, "email:YYYY-MM-DD HH")) — hourly rolling
  const now    = new Date();
  ...
  try {
    ...
    const data = await res.json();
    await logEvent(env, 'build', 'domain_proxy', res.ok ? 'success' : 'failure', { action, domain: `${sld}.${tld}`, error: data?.error });
    return data;
  } catch (e) {
    await logEvent(env, 'build', 'domain_proxy', 'failure', { action, error: e.message });
    throw e;
  }
}
...
function buildPass1UserPrompt(client, archetype) {
  ...
  "coverage_areas": ["${area}", "nearby suburb", "nearby suburb", "nearby suburb", "nearby suburb", "nearby suburb"],
  "unsplash_queries": {
    "hero": "${industry} professional south africa action",
    "about": "${industry} team people south africa",
    "services": "${industry} work detail tools south africa"
  }
}`,
  ...
  "team_member3_name": "plausible SA name", "team_member3_title": "e.g. Specialist",
  ...
  "about_p1": "paragraph — the firm founding story",
  "about_p2": "paragraph — their approach and values",
  "about_p3": "paragraph — why clients choose them",
  "testimonials": [
    {"name": "Initial only e.g. T.M.", "quote": "authentic SA testimonial", "matter": "e.g. Family Law"},
  ...
  "unsplash_queries": {
    "hero": "${industry} professional office south africa formal",
    "about": "${industry} team meeting professionals south africa",
    "services": "${industry} document consultation south africa"
  }
}`,
  ...
  "hero_mood_line": "atmospheric one-liner",
  ...
  "gallery_section_tag": "e.g. Our Work", "gallery_h2": "e.g. See What We Do",
  ...
  "address_line1": "${area}, South Africa",
  "unsplash_queries": {
    "hero": "${industry} interior ambiance south africa warm",
    "about": "${industry} owner staff south africa welcoming",
    "services": "${industry} detail product close-up south africa"
  }
}`,
  ...
  "hero_h1_line2": "the local advantage line",
  ...
  "since_year": "e.g. 2004", "trade": "${industry}",
  "owner_name": "inferred from business name",
  "about_tagline": "one neighbourhood phrase",
  "services_section_tag": "e.g. What We Stock | What We Do",
  ...
  "badges": ["e.g. Family Owned", "e.g. ${area} Born", "e.g. Open 7 Days", "e.g. Cash & Card"],
  "about_section_tag": "e.g. Our Story",
  "about_headline": "community headline",
  "about_pull_quote": "one line that captures neighbourhood spirit",
  "about_p1": "paragraph — the founding story",
  "about_p2": "paragraph — the community connection",
  "testimonials": [
  ...
  "address_line1": "${area}, South Africa",
  "unsplash_queries": {
    "hero": "${industry} shop storefront south africa local",
    "about": "${industry} owner community south africa friendly",
    "services": "${industry} products shelves south africa"
  }
}`,
  ...
  "clients_served": "e.g. 500+ homeowners and businesses",
  "years_active": "e.g. 12",
  "owner_name": "inferred from business name",
  ...
  "testimonials": [
    {"name": "SA name", "quote": "results-focused testimonial", "result": "e.g. Done in 3 days, perfect finish"},
  ...
  "unsplash_queries": {
    "hero": "${industry} finished work result south africa quality",
    "about": "${industry} craftsman professional working south africa",
    "services": "${industry} before after transformation south africa"
  }
}`,
  };
  ...
  return `Generate website content for this South African business. Return ONLY the JSON — no other text.
